...
## 🛠 開發方法論：Mock-First 三段式

嚴禁在單一 Commit 中同時完成 UI、邏輯與整合。每個功能必須遵循以下開發路徑：

1.  **Step 1: UI & Mock** — 使用假資料刻出 UI，完成 i18n 與互動。
2.  **Step 2: State & Logic** — 引入 Riverpod，撰寫單元測試 (Unit Test)，使用 Mock Repository。
3.  **Step 3: Integration** — 實作真實 Firebase 儲存與 Remote Config 串接。
